...
export default async function handler(req,res){
  ...
  try{
    ...
    if(!r.ok){
      console.error('nexo-realtime-call failed',r.status,answer);
      return res.status(r.status).send(answer||'realtime_call_failed');
    }
    res.setHeader('Content-Type','application/sdp');
    res.setHeader('Cache-Control','no-store');
    return res.status(200).send(answer);
  }catch(e){
    console.error('nexo-realtime-call error',e);
